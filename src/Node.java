...
/* NOTE:
        ...
        If we try to generate all possibilities from that turn, their number would be 12! i.e. 479 001 600.
        ...
        for the rest of the party, which is more reasonable and can be stored in memory.

    NOTE:
       turn represent the absolute turn in the party, thus it ranges from 1 to 12.
       It means that:
          - if turn%2 == 1, it represents a turn of the blue player
          - if turn%2 == 0, it
          - (turn+1)/2 represents the pawn value played by the player at this turn. represents a turn of the red player
...
public class Node
{
    ...
    static byte BLUE_WINS = 1;
    static byte RED_WINS = 2;
    ...
    Node(int idCell, int turn)
    {
        ...
        this.turn = (byte)turn;
        if      (turn == 1) children = new Node[1];
        else if (turn < 12) children = new Node[13-turn];
        else children = null;
        nbChildren = 0;
        result = NO_RESULT;
    }
